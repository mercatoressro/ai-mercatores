  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < connectionDist) {
            ...
            const alpha = clampAlpha(distFade * centerFade * 3.5, 1);
      ...
      for (const n of nodes) {
        ...
        const alpha = clampAlpha(pulse * centerFade * 3.2, 1);

        if (alpha < 0.01) continue;

        // Soft glow
        const grad = ctx.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.radius * 5.5);
        grad.addColorStop(0, nodeColor(alpha * 0.72));
        grad.addColorStop(1, nodeColor(0));
        ctx.beginPath();
        ctx.arc(n.x, n.y, n.radius * 5.5, 0, Math.PI * 2);
        ctx.fillStyle = grad;
        ctx.fill();
